#' @title  runIP4CI
#' @description run IP4CI main function to run the analysis using two objects
...
runIP4CI <- function(expr_dir, id.list,obj.fname.list,
                     convertG.list,
                     typeColumnToUse.list,assaySlotToUse.list,
                     genes2keep,
                     processObjOpt,
                     ...
                     topRanked)
{
  ...
  createDir(res_dir)
  run_00_preprocessData(data_dir,res_dir, id.list,obj.fname.list,convertG.list,processObjOpt,typeColumnToUse.list,assaySlotToUse.list)
  ################################################################################
 ...
 data_dir =  paste0(expr_dir, '03_sampleScoring/')
  ...
  # id.list[[1]]
  pattern_s = paste0(filter_opt,filter_cutoff,sample_cutoff,'*',id.list[[1]],'*.RData')
  ...
  write.table(merged_score1, file = paste0(res_dir,id.list[[1]],'_Merged.txt'), sep = "\t", quote = FALSE, row.names = TRUE)
  merged_score1.Pinfo = getPathwaysInfo(rownames(merged_score1))
  write.table(merged_score1.Pinfo, file =paste0(res_dir,id.list[[1]],'_PathwaysInfo.txt'), sep = "\t", quote = FALSE, row.names = TRUE)
  ...
  visData(df.s,annot.s,p.df.s,paste0(res_dir,id.list[[1]],'_visData.pdf'))
  nVarF=nrow(df.s)
  df.s.obj = df2obj(df.s,annot.s,nVarF)
  dim_name = 'umap'
  visObj(df.s.obj,dim_name,paste0(res_dir,id.list[[1]],'_vis_varF_',nVarF,'.pdf'))
  # # id.list[[2]]
  pattern_s = paste0(filter_opt,filter_cutoff,sample_cutoff,'*',id.list[[2]],'*.RData')
  mergingFiles_lst =  list.files(data_dir, pattern=glob2rx(pattern_s))
  merged_score2 = mergeMultiFiles(paste0(data_dir,mergingFiles_lst))
  write.table(merged_score2, file = paste0(res_dir,id.list[[2]],'_Merged.txt'), sep = "\t", quote = FALSE, row.names = TRUE)
  merged_score2.Pinfo = getPathwaysInfo(rownames(merged_score2))
  write.table(merged_score2.Pinfo, file =paste0(res_dir,id.list[[2]],'_PathwaysInfo.txt'), sep = "\t", quote = FALSE, row.names = TRUE)
  ...
  visData(df.s,annot.s,p.df.s,paste0(res_dir,id.list[[2]],'_visData.pdf'))
  nVarF=nrow(df.s)
  df.s.obj = df2obj(df.s,annot.s,nVarF)
  dim_name = 'umap'
  visObj(df.s.obj,dim_name,paste0(res_dir,id.list[[2]],'_vis_varF_',nVarF,'.pdf'))
  # id.list[[1]] & id.list[[2]]
  ...
  write.table(merged_score12.Pinfo, file =paste0(res_dir,id.list[[1]],'_',id.list[[2]],'_PathwaysInfo.txt'), sep = "\t", quote = FALSE, row.names = TRUE)
  ...
  visData(df.s,annot.s,p.df.s,paste0(res_dir,id.list[[1]],'_',id.list[[2]],'_visData.pdf'))
  nVarF=nrow(df.s)
  df.s.obj = df2obj(df.s,annot.s,nVarF)
  dim_name = 'umap'
  visObj(df.s.obj,dim_name,paste0(res_dir,id.list[[1]],'_',id.list[[2]],'_vis_varF_',nVarF,'.pdf'))
  ...
  s1 = rownames(subset(annot, annot$dataset_name == id.list[[1]]))
  s2 = rownames(subset(annot, annot$dataset_name == id.list[[2]]))
  if(ObjType == 'merged')
  {
    x=merged_score12[,s1]
    y=merged_score12[,s2]
  ...
  p.df.s= merged_score12.Pinfo[merged_score12.Pinfo$p_id %in% rownames(merged_score12),]
  df.s = merged_score12[rownames(merged_score12) %in% rownames(p.df.s),]
  visData(df.s,annot.s,p.df.s,paste0(res_dir,id.list[[1]],'_',id.list[[2]],'_visDataAvg.pdf'))
  df.s.obj = df2obj(df.s,annot.s,nVarF)
  saveRDS(df.s.obj,paste0(res_dir,id.list[[1]],'_',id.list[[2]],'_obj_varF_',nVarF,'.RDS'))
  ...
  saveRDS(obj.x,paste0(res_dir,id.list[[1]],'_',ObjType,'obj_varF_',nVarFx,'.RDS'))
  nVarFy = nrow(y)
  obj.y =df2obj(y,annot[s2,],nVarFy)
  saveRDS(obj.y,paste0(res_dir,id.list[[2]],'_',ObjType,'obj_varF_',nVarFy,'.RDS'))
  dim_name = 'umap'
  visObj(obj.x,dim_name,paste0(res_dir,id.list[[1]],'_vis_varF_',nVarFx,'.pdf'))
  dim_name = 'umap'
  visObj(obj.y,dim_name,paste0(res_dir,id.list[[2]],'_vis_varF_',nVarFy,'.pdf'))
 ...
 embed2use = 'uv'
  ...
  run_06_similarityCalculation(cca_res,annot,id.list=c(id.list[[1]],id.list[[2]]),ct='all',paste0(res_dir,ObjType,'_',num.cc,'_',nVarF,'_',selected_dims_opt,selected_dims_opt_cutoff))
